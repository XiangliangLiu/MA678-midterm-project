...
Number of reviews for different rooms:
```{r}
...
Room type with rating and price
```{r}
...
```{r}
ggplot(data = room_type, aes(x= reorder(room_type,-Avg_rating),y = Avg_rating)) + geom_bar(stat = "identity", fill = "sky blue") + ggtitle("Average rating with different room types") + ylab("Average price") + theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
...
```{r}
#cor(BostonAirbnb[,c()] )
# Bostonmap<-qmap("Boston", zoom=12, "osm")
```
